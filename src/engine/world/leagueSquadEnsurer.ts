...
const MIN_JUNIOR_SQUAD_SIZE = 12;
  ...
  for (const countryCode of unlockedCountryCodes) {
    ...
    const countryLeagues = leagues.filter(
      (l) =>
        ...
        (COMPETITION_TIERS.includes(l.tier) || l.tier === 'junior'),
    );

    for (const league of countryLeagues) {
      const minSize = league.tier === 'junior' ? MIN_JUNIOR_SQUAD_SIZE : MIN_SQUAD_SIZE;
      ...
      for (const club of leagueClubs) {
        ...
        if (current >= minSize) continue;

        const rank = sorted.findIndex((c) => c.id === club.id);
        const needed = minSize - current;
